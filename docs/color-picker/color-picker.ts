class InputClass {
    ...
    private update(): void {
        ...
        function hsl2rgb(hsl: [number, number, number]): [number, number, number] {
            ...
            if (0 <= hue && hue < 60) { r = chroma; g = mid; b = 0 }
            else if (60 <= hue && hue < 120) { r = mid; g = chroma; b = 0 }
            else if (120 <= hue && hue < 180) { r = 0; g = chroma; b = mid }
            else if (180 <= hue && hue < 240) { r = 0; g = mid; b = chroma }
            else if (240 <= hue && hue < 300) { r = mid; g = 0; b = chroma }
            else if (300 <= hue && hue < 360) { r = chroma; g = 0; b = mid }
